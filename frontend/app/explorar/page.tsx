            ...
            {!me || !wishlist ? (
              ...
              <div className="space-y-2.5">
                {wishlist.slice(0, 4).map((t) => (
                  <Link key={t.id} href={`/moment/${t.id}`} className="flex items-baseline justify-between gap-2">
        ...
        <div className="min-w-0 space-y-3">
          ...
          {events.map((e, i) => {
            ...
            const row = compact ? (
              <Link
                key={e.id}
                href={e.template ? `/moment/${e.template.id}` : '/explorar'}
