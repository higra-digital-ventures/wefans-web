  ...
  return (
    <Link href={`/vitrine/${s.id}`} className="rounded-2xl block  border border-line bg-panel p-5 transition-colors hover:border-accent/40">
      ...
      <p className="text-sm text-muted">
        {s.itemCount} Momentos{s.ownerUsername ? ` · @${s.ownerUsername}` : ''}
        {!s.public ? ' · privada' : ''}
      </p>
...
export default async function VitrinesPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-12">
      ...
      <p className="mb-8 text-muted">Álbuns curados dos seus Momentos.</p>
        ...
        <section className="mb-10">
          ...
          {mine.length === 0 ? (
            <EmptyState
              ...
              hint="Monte uma seleção dos seus melhores Momentos e mostre para a galera."
            />
          ) : (
